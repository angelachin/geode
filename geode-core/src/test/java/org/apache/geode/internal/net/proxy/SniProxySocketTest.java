        ...
        while (!Thread.currentThread().isInterrupted()) {
          ...
          while (iterator.hasNext()) {
            ...
            if (key.isReadable()) {
              ...
              SocketChannel endpointSocketChannel = SocketChannel.open(new InetSocketAddress(hostname, internalServerSocket.getLocalPort()));
              endpointSocketChannel.configureBlocking(false);
              // TODO need different selector endpointSocketChannel.register(proxySelector, SelectionKey.OP_READ);
            }
          }
        }
      } catch (Exception e) {
...
//    proxyServerSocket.close();
  ...
  public void connectInterceptsToProxy() throws IOException, NoSuchAlgorithmException {
    ...
    socket.connect(getLocalSocketAddress(internalServerSocket));
    ...
    final SSLSocket sslSocket = (SSLSocket) SSLContext.getDefault().getSocketFactory().createSocket(socket,
